...
class Article extends Model
{
    ...
    public function scopeSearch($query, $request)
    {
        ...
        if (!empty($category_id)) {
            $query->Join('articles_categories', function ($join) use ($category_id) {
                $join->on('articles_categories.article_id', '=', 'articles.id')->where('articles_categories.category_id', '=', $category_id);
            });
        }
        $query->when($query != null, function ($query) use ($search_by) {
